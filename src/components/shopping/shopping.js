  ...
  render() {
    const dataa = data.items.map(data => {
      if (this.state.filtersize.length <= 0) {
        return (
          <div className="four-col" key={data.id}>
            ...
            <div className="name-wrap">
              ...
              {/* <button className="add-cart" onClick={() => this.addtocart(data.id)}>
                Add To Cart
              </button> */}
              <Button onClick={() => this.addtocart(data.id, true)}>
                Add to Cart
              </Button>
            </div>
          </div>
        ...
        return this.state.filtersize.includes(data.size) ? (
          <div className="four-col" key={data.id}>
            <div className="image-wrap">
              <Link to={{ pathname: `/${data.id}` }}>
                <img src={data.imageurl} alt="shopping" />
      ...
      <React.Fragment>
        <div className="container">
          <div className="filter-sizes">
            <div
              className={
                this.state.filtersize.includes("xl") ? "size active" : "size"
              }
              id="xl"
              onClick={this.filtersizes.bind(this)}
            >
              {" "}
              xl{" "}
            </div>
            <div
              className={
                this.state.filtersize.includes("l") ? "size active" : "size"
              }
              id="l"
              onClick={this.filtersizes.bind(this)}
            >
            ...
            <div
              className={
                this.state.filtersize.includes("m") ? "size active" : "size"
              }
              id="m"
              onClick={this.filtersizes.bind(this)}
            >
        ...
        {this.state.cartstatus ? <Cart array={this.state.cart} /> : null}
      </React.Fragment>
    );
  }
}
